...
const SECTIONS = [
  { label: 'First Class', fromRow: 1,  toRow: 2,  badge: 'text-amber-700 bg-amber-50 border-amber-200' },
  { label: 'Business',    fromRow: 3,  toRow: 6,  badge: 'text-indigo-700 bg-indigo-50 border-indigo-200' },
  { label: 'Economy',     fromRow: 7,  toRow: 30, badge: 'text-slate-600  bg-slate-50  border-slate-200'  },
]

export function getSeatSurcharge(seat) {
  return Number(seat?.extra_price ?? 0)
}
...
export function getSeatDisplay(seat) {
  return `${seat.row_number}${seat.seat_letter}`
}
...
function SeatButton({ displayKey, isOccupied, isSelected, canSelectMore, onClick }) {
  ...
  let cls
  if (isOccupied) {
    cls = 'bg-slate-200 border-slate-300 cursor-not-allowed'
  } else if (isSelected) {
    cls = 'bg-blue-600 border-blue-700 text-white shadow-md scale-105'
  } else if (canSelectMore) {
    cls = 'bg-white border-slate-300 hover:bg-emerald-50 hover:border-emerald-400 hover:scale-110 cursor-pointer'
  } else {
    cls = 'bg-white border-slate-200 opacity-40 cursor-not-allowed'
  }
  ...
  return (
    <button
      ...
      className={`w-7 h-7 rounded-md border text-[9px] font-bold transition-all duration-150 ${cls}`}
    >
      {isSelected ? '✓' : ''}
    </button>
  )
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-2 mb-5">
        {[
          ...
          { label: 'Occupied',  cls: 'bg-slate-200 border-slate-300' },
      ...
      <div className="overflow-x-auto pb-1">
        <div className="inline-block min-w-[268px]">
          {/* Column headers */}
          <div className="flex items-center gap-1 mb-3">
            <div className="w-7 shrink-0" />
            {['A', 'B', 'C'].map(c => (
              <div key={c} className="w-7 text-center text-xs font-bold text-slate-400 tracking-wider">{c}</div>
            ))}
          ...
          {SECTIONS.map(({ label, fromRow, toRow, badge }) => {
